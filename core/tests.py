...
class UserModelTests(TestCase):
    ...
    valid_create_user_kwargs = {
        "email": valid_emails[0][0],
        "first_name": valid_first_names[0][0],
        "last_name": valid_last_names[0][0],
        "username": valid_usernames[0][0],
    }

    def assert_user_created_for_params(self, create_user_kwargs: Dict[str, str],
                                       key_to_assert: str, values_to_try: List[Tuple[str, str]]):
        for value, reason in values_to_try:
    ...
    def assert_user_created_error_for_params(self, raises, create_user_kwargs: Dict[str, str],
        ...
        for value, reason in values_to_try:
            create_user_kwargs[key_to_assert] = value
            self.assertRaises(raises, User.objects.create_user,
                              create_user_kwargs, reason)
